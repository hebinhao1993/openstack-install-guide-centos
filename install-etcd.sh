...
sed -i -e 's/^ETCD_LISTEN_PEER_URLS.*/ETCD_LISTEN_PEER_URLS="http://10.0.2.5:2380"/' /etc/etcd/etcd.conf


sed -i -e 's/^#ETCD_LISTEN_CLIENT_URLS/ETCD_LISTEN_CLIENT_URLS/' /etc/etcd/etcd.conf
sed -i -e 's/^ETCD_LISTEN_CLIENT_URLS.*/ETCD_LISTEN_CLIENT_URLS="http://10.0.2.5:2379"/' /etc/etcd/etcd.conf
...
sed -i -e 's/^ETCD_INITIAL_ADVERTISE_PEER_URLS.*/ETCD_INITIAL_ADVERTISE_PEER_URLS="http://10.0.2.5:2380"/' /etc/etcd/etcd.conf

sed -i -e 's/^#ETCD_ADVERTISE_CLIENT_URLS/ETCD_ADVERTISE_CLIENT_URLS/' /etc/etcd/etcd.conf
sed -i -e 's/^ETCD_ADVERTISE_CLIENT_URLS.*/ETCD_ADVERTISE_CLIENT_URLS="http://10.0.2.5:2379"/' /etc/etcd/etcd.conf

sed -i -e 's/^#ETCD_INITIAL_CLUSTER/ETCD_INITIAL_CLUSTER/' /etc/etcd/etcd.conf
sed -i -e 's/^ETCD_INITIAL_CLUSTER.*/ETCD_INITIAL_CLUSTER="controller=http://10.0.2.5:2380"/' /etc/etcd/etcd.conf
